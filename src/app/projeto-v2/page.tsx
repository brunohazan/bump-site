...
export default function ProjectV2Page() {
  return (
    <>
      ...
      <section className="section-space"><div className="site-container grid gap-10 lg:grid-cols-2"><div><Eyebrow>03 · Entregue nesta evolução</Eyebrow><ul className="mt-6 grid gap-3">{delivered.map((item) => <li key={item} className="flex gap-3 rounded-sm border border-line-1 bg-ink-card p-4"><span className="text-accent">✓</span><span>{item}</span></li>)}</ul></div><div><Eyebrow>04 · Próximas etapas</Eyebrow><div className="rounded-sm border border-line-2 bg-ink-card p-7"><ol className="grid gap-5">{["Revisão visual e de copy com o cliente", "Substituição de fotos e logo provisórios", "Conexão segura dos formulários com LGPD", "SEO/AEO, analytics e validação de performance", "CMS, domínio e deploy de produção"].map((item, index) => <li key={item} className="flex gap-4"><span className="font-mono text-xs text-accent">0{index + 1}</span><span className="font-semibold">{item}</span></li>)}</ol></div></div></div></section>
