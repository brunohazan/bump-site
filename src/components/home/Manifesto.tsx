...
export function Manifesto() {
  return (
    <section
      id="manifesto"
      className="grid grid-cols-[1fr_minmax(0,1080px)_1fr] px-10 pt-40 pb-[140px]"
    >
      <div className="col-start-2">
        ...
        <h2
          data-reveal
          className="m-0 mb-[72px] max-w-[22ch] text-[clamp(36px,5vw,68px)] leading-[1.08] font-extrabold tracking-[-0.025em]"
        >
        ...
        <div className="grid grid-cols-4 gap-px border border-line-1 bg-line-1">
          {STATS.map((stat) => (
            <div
              key={stat.label}
              data-reveal
              className="flex flex-col gap-2.5 bg-ink-card px-7 py-8"
            >
              <span className="text-[44px] font-extrabold tracking-[-0.03em]">
                {stat.value}
                {stat.accent && (
