...
export function ProductLineCard({ line }: { line: ProductLine }) {
  return (
    <article data-reveal className="group flex h-full flex-col overflow-hidden rounded-sm border border-line-1 bg-ink-card transition-colors hover:border-line-4">
      <Link href={`/linhas/${line.slug}`} className="relative block min-h-56 overflow-hidden bg-ink-frame">
        ...
        <span aria-hidden="true" className="product-sweep" />
